...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#possibility").text(possJSON["${tran.stage}"])

            changeStage(currentStage)

        });

        var possJSON = eval("("+"${possibility}"+")");   //获取可能性json

        function changeStage(text) {
            if(possJSON[text] == 0){
                <c:forEach items="${stageList}" var="s">
                    if(possJSON["${s.text}"] != 0){
                        $("#${s.id}").addClass("glyphicon-record")
                        $("#${s.id}").css("color","#000000")
                    }else if("${s.text}" == text){   //红×
                        $("#${s.id}").addClass("glyphicon-remove")
                        $("#${s.id}").css("color","red")
                    }else {
                        $("#${s.id}").addClass("glyphicon-remove")
                        $("#${s.id}").css("color","#000000")
                ...
                <c:forEach items="${stageList}" var="s">
                    if("${s.text}" == text){        /*进行中*/
                        $("#${s.id}").addClass("glyphicon-map-marker")
                        $("#${s.id}").css("color","#90F790")
                        flag = true;
                    }else if(possJSON["${s.text}"] == 0){       /*交易失败（黑叉）*/
                        $("#${s.id}").addClass("glyphicon-remove")
                        $("#${s.id}").css("color","#000000")
                    }else if(flag){                             /*未完成    未完成肯定是在进行中后面的*/
                        $("#${s.id}").addClass("glyphicon-record")
                        $("#${s.id}").css("color","#000000")
                    }else{                                      /*已完成*/
                        $("#${s.id}").addClass("glyphicon-ok-circle")
                        $("#${s.id}").css("color","#90F790")
                    }
                </c:forEach>
            }
        }
        function clickStage(text) {
            if(text == currentStage){
                //点击了正在进行中的图标
                return;
            }
            if(confirm("确定进行到 "+text+" 阶段吗")){
                $.ajax({
                    ...
                    success: function (data) {
                        //[{success:boolean},{editTime:修改时间}]
                        if(data.success){
                            //变更图标  首先去除图标的样式 在调用changeStage方法重新赋予样式 更改修改者修改事件，阶段以及可能性
                            //刷新交易历史   这步略

                            <c:forEach items="${stageList}" var="s">
                            $("#${s.id}").removeClass("glyphicon-ok-circle")
                            $("#${s.id}").removeClass("glyphicon-map-marker")
                            $("#${s.id}").removeClass("glyphicon-record")
                            $("#${s.id}").removeClass("glyphicon-remove")
                            </c:forEach>
                            changeStage(text)

                            $("#stage").text(text)
                            $("#possibility").text(possJSON[text])   //刷新可能性
                            $("#editBy").text("${user.name}")   //刷新修改者 从sessionScope取值
                            $("#editTime").text(data.editTime)   //刷新修改时间
...
<div style="position: relative; left: 40px; top: -50px;">
    阶段&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <c:forEach items="${stageList}" var="s">
        <span id="${s.id}" onclick="clickStage('${s.text}')" class="glyphicon mystage" data-toggle="popover" data-placement="bottom"
...
<div style="position: relative; top: 0px;">
    ...
    <div style="position: relative; left: 40px; height: 30px; top: 70px;">
        ...
        <div style="width: 500px;position: relative; left: 200px; top: -20px;"><b id="editBy">${tran.editBy}&nbsp;&nbsp;</b><small id="editTime"
    ...
    <div style="position: relative; top: 100px; left: 40px;">
        ...
        <div style="position: relative;top: 0px;">
            <table id="activityTable" class="table table-hover" style="width: 900px;">
                ...
                <tbody>
                <%--应使用ajax请求--%>
                <c:forEach items="${tranHisList}" var="th">
                    <tr>
                        <td>${th.stage}</td>
                        <td>${th.money}</td>
                        <td>**</td>
